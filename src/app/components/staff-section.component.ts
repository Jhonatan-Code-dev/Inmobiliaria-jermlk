...
import { StaffMember, StaffPayload, StaffFilters } from '../core/staff/staff.models';
  ...
  template: `
    <section class="max-w-7xl mx-auto space-y-8 p-4">
      ...
      <div class="bg-white p-6 rounded-[1.5rem] border border-slate-100 shadow-sm">
        <form [formGroup]="searchForm" (ngSubmit)="loadStaff()" class="flex flex-wrap gap-4">
                  ...
                  <tr class="group hover:bg-slate-50/80 transition-all">
                    <td class="px-8 py-6">
                      <div class="flex items-center gap-4">
                        ...
                        <span class="text-sm font-black text-slate-800 uppercase tracking-tight">{{ item.usuario }}</span>
                      </div>
                    </td>
                    <td class="px-8 py-6">
                      <span class="text-xs font-bold text-slate-600 uppercase">{{ item.rol }}</span>
                    </td>
                    <td class="px-8 py-6">
          ...
          <div class="relative w-full max-w-xl bg-white rounded-[2.5rem] shadow-2xl animate-zoom overflow-hidden">
            ...
            <form [formGroup]="staffForm" (ngSubmit)="submitStaff()" class="p-10 space-y-6">
              <div class="space-y-1.5">
                ...
                <input type="text" formControlName="usuario" class="w-full h-12 px-4 rounded-xl bg-slate-50 border-transparent outline-none font-bold text-slate-900"/>
              </div>
              @if (!editingId()) {
                <div class="space-y-1.5">
                  <label class="text-[10px] font-black uppercase tracking-widest text-slate-400">Contraseña</label>
                  <input type="password" formControlName="password" class="w-full h-12 px-4 rounded-xl bg-slate-50 border-transparent outline-none font-bold text-slate-900"/>
                </div>
              }
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                  <label class="text-[10px] font-black uppercase tracking-widest text-slate-400">Rol</label>
                  <select formControlName="rol_id" class="w-full h-12 px-4 rounded-xl bg-slate-50 border-transparent outline-none font-bold text-slate-700">
                ...
                <div class="space-y-1.5">
                  ...
                  <select formControlName="estado" class="w-full h-12 px-4 rounded-xl bg-slate-50 border-transparent outline-none font-bold text-slate-700">
              ...
              <div class="flex gap-3 pt-6">
                ...
                <button type="submit" [disabled]="isSaving()" class="flex-1 h-12 rounded-xl bg-slate-900 text-white text-xs font-black uppercase tracking-widest shadow-xl hover:bg-emerald-600 transition-all">
      ...
      @if (pendingDelete()) {
        <div class="fixed inset-0 z-[150] flex items-center justify-center p-4">
          ...
          <div class="relative w-full max-w-sm bg-white p-8 rounded-[2rem] shadow-2xl animate-zoom text-center">
            ...
            <p class="text-xs font-medium text-slate-500 mt-2 mb-8">Esta acción no se puede deshacer.</p>
...
export class StaffSectionComponent implements OnInit {
  ...
  loadStaff(): void {
    ...
    this.staffService.list(this.searchForm.getRawValue())
      .pipe(finalize(() => this.isLoading.set(false)))
      .subscribe(res => this.staff.set(res.datos));
  }

  openComposer(): void {
    this.editingId.set(null);
    this.staffForm.reset({ rol_id: 1, estado: 'activo' });
  ...
  submitStaff(): void {
    ...
    const payload = this.editingId() ? { rol_id: val.rol_id, estado: val.estado as any } : val as StaffPayload;
    
    const req = this.editingId() ? this.staffService.update(this.editingId()!, payload) : this.staffService.create(payload as StaffPayload);
    
    req.pipe(finalize(() => { this.isSaving.set(false); this.closeComposer(); }))
       .subscribe(() => { this.setFeedback('success', 'Staff actualizado.'); this.loadStaff(); });
  }

  openDelete(item: StaffMember): void { this.pendingDelete.set(item); }
  confirmDelete(): void {
